...
Usage: make -f makefile [GOALS] CONF=[makeconf] TARG=[release/debug/all]
    COMP=[gpp/mingw/msvc] ARCH=[64/32/all] CRT=[static/dynamic/all]
    OPTS=["custom opts"] SRCS=[files]
...
GOALS :
    ...
    dist          - Make and link the output (same as all), then run the create
                    distribution shell script.
    ...
    run           - Execute the binary output. The OPTS value is used to supply
                    run arguments.
    dbg           - Execute the binary under the debugger. The OPTS value is used to
...
## COMMON VARIABLES ## 
An additional configuration file (make-vars.conf) used to provide a common place to set
